...
static int dfs_uffs_close(struct dfs_file *file)
{
    ...
    if (oflag & O_DIRECTORY)
    {
        ...
        if (uffs_closedir((uffs_DIR *)(file->vnode->data)) < 0)
            return uffs_result_to_dfs(uffs_get_error());
...
static ssize_t dfs_uffs_write(struct dfs_file *file,
                          const void *buf,
                          size_t len)
{
    int fd;
                         ...
                         off_t offset)
{
    ...
    {
        uffs_rewinddir((uffs_DIR *)(file->vnode->data));
        result = uffs_seekdir((uffs_DIR *)(file->vnode->data), offset / sizeof(struct dirent));
        if (result >= 0)
        {
            file->pos = offset;
            return offset;
    ...
    {
        result = uffs_seek((int)(file->vnode->data), offset, USEEK_SET);
        if (result >= 0)
            return offset;
    }
