...
static int dfs_yfile_open(struct dfs_file *file)
{
    ...
    fs = (struct dfs_filesystem *)file->vnode->data;
...
static int dfs_yfile_close(struct dfs_file *file)
{
    ...
    {
        if (yaffs_closedir((yaffs_DIR *)(file->vnode->data)) < 0)
            return yaffsfs_GetLastError();
        return 0;
    }

    /* regular file operations */
    fd = (int)(file->vnode->data);
...
static ssize_t dfs_yfile_read(struct dfs_file *file, void *buf, size_t len)
{
    int fd;
    ssize_t char_read;
...
static ssize_t dfs_yfile_write(struct dfs_file *file, const void *buf, size_t len)
{
    int fd;
    ssize_t char_write;
...
static off_t dfs_yfile_lseek(struct dfs_file *file, rt_off_t offset)
{
    int fd;
    off_t result;
