...
int wireless_wifi_image_size = 355255;
extern const uint8_t wireless_wifi_image[];
...
//		size = wireless_wifi_image_size;//add replace
...
//		memcpy(buffer, &wireless_wifi_image[offset], buffer_size);//add replace
...
static void wifi_init_thread_entry(void *parameter)
{
	...
	/* set wifi work mode */
	rt_wlan_set_mode(RT_WLAN_DEVICE_STA_NAME, RT_WLAN_STATION);//进入配置后烂在这里了，原因是sdio_blk为0，似乎被哪里更改
	/* NEED TODO !!! */
	/* rt_wlan_set_mode(RT_WLAN_DEVICE_AP_NAME, RT_WLAN_AP); */
...
{
#define AP6212_WL_REG_ON    GET_PIN(G, 2)
