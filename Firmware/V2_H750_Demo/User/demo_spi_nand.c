...
*********************************************************************************************************
*	函 数 名: sf_PageWrite
*	功能说明: 向一个page内写入若干字节。字节个数不能超出页面大小（2K)
*	形    参:  	_pBuf : 数据源缓冲区；
...
uint32_t sf_ReadID(void)
{
	...
	id1 = g_spiRxBuf[2];					/* 读ID的第1个字节 */
	id2 = g_spiRxBuf[3];					/* 读ID的第2个字节 */
	id3 = g_spiRxBuf[4];					/* 读ID的第3个字节 */
	sf_SetCS(1);									/* 禁能片选 */
...
void sf_ReadInfo(void)
{
	/* 自动识别串行Flash型号 */
	{
		...
		switch (g_tSF.ChipID)
		{
			...
			case W25N01GV_ID:
				strcpy(g_tSF.ChipName, "W25N01GV");
				g_tSF.TotalSize = 128 * 1024 * 1024;	/* 总容量 = 128M */
				g_tSF.PageSize = 2 * 1024;			/* 页面大小 = 2K */
				break;	
